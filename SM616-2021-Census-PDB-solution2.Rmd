...
The design effect is high at `r bfgso_arr$value`.
...
st_nm_arr_design$sample_design %>% 
  mutate(s_rate = 1000*n2_field/pop) %>%
  select(-pop, -black, -hisp, -RR) %>% 
  mutate(
    definition = case_when(
      strata4 == 1 ~ paste0("> ", round(st_nm_arr_design$above_black*100,2), "% NH Black"),
      strata4 == 2 ~ paste0("> ", round(st_nm_arr_design$above_hisp*100,2), "% Hisp"),
      strata4 == 3 ~ paste0("> ", round(st_nm_arr_design$above_minor*100,2), "% minority"),
      strata4 == 4 ~ "Mostly NH White")
  ) %>% select(strata4,definition, tracts, s_rate, starts_with("n2")) %>%
  mutate(across(where(is.numeric),round)) %>%
  janitor::adorn_totals("row") %>%
  maybe_kable()
```
...
st_nm2_arr_design$sample_design %>% 
  ...
  mutate(
    definition = case_when(
      strata4 == 1 ~ paste0("> ", round(st_nm2_arr_design$above_black*100,2), "% NH Black"),
      strata4 == 2 ~ paste0("> ", round(st_nm2_arr_design$above_hisp*100,2), "% Hisp"),
      strata4 == 3 ~ paste0("> ", round(st_nm2_arr_design$above_minor*100,2), "% minority"),
      strata4 == 4 ~ "Mostly NH White")
  ) %>% select(strata4,definition, tracts, s_rate, starts_with("n2")) %>%
  mutate(across(where(is.numeric),round)) %>%
  janitor::adorn_totals("row") %>%
